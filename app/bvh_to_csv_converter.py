...
class Viewer:
    def __init__(self, viewer, config):
        ...
        self.csv_config = csv_utils.get_csv_config_for_target(self.config.get('retarget_target', 'unitree_g1'))
        ...
        self.retarget_target_options = ['unitree_g1', 'unitree_h1']
        ...
        ) if self.config.get('retarget_target', 'unitree_g1') in self.retarget_target_options else 0
        ...
        self.setup_robot_model(self.retarget_target_options[self.retarget_target_idx])

    def setup_robot_model(self, retarget_target):
        robot_builder = create_robot_builder(retarget_target)
        ...
        self.csv_config = csv_utils.get_csv_config_for_target(retarget_target)

    def set_retarget_target(self, retarget_target):
        self.setup_robot_model(retarget_target)
        self.robot_csv_animation_buffers = [None for _ in range(self.num_robots)]
    ...
    def ui_scene_options(self, ui):
        ...
        if ui.collapsing_header("Motion", flags=ui.TreeNodeFlags_.default_open):
            ...
            ui.text("Target:")
            ui.same_line()
            ui.text_colored(ui.ImVec4(0.6, 0.8, 1.0, 1.0), self.retarget_target_options[self.retarget_target_idx])

            for idx, target in enumerate(self.retarget_target_options):
                if idx > 0:
                    ui.same_line()
                selected = idx == self.retarget_target_idx
                if selected:
                    ui.begin_disabled()
                if ui.button(target):
                    self.retarget_target_idx = idx
                    self.set_retarget_target(target)
                if selected:
                    ui.end_disabled()
            ...
            _, self.retarget_solver_idx = ui.combo(
                "##RetargetSolver",
                self.retarget_solver_idx,
                self.retarget_solver_options)
    ...
    def batched_retargeting(self):
        ...
        csv_config = csv_utils.get_csv_config_for_target(retarget_target)
